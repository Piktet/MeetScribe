...
func TestChatConnection_Connect(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		srv := newServer(t, http.StatusOK, `{"access_token":"tok-1","expires_at":4102444800}`)
	...
	t.Run("auth error", func(t *testing.T) {
		srv := newServer(t, http.StatusUnauthorized, `unauthorized`)
		conn := New(srv.URL, "uid-1", "authkey")
		assert.Error(t, conn.Connect(ctx))
		_, err := conn.GetToken()
		assert.Error(t, err)
	})
...
func TestChatConnection_GetTokenWithoutConnect(t *testing.T) {
...
// TestChatConnection_Reconnect проверяет автоматическое переподключение
// после истечения срока жизни токена.
func TestChatConnection_Reconnect(t *testing.T) {
